...
export function useChat(sessionId: string) {
  ...
  const loadHistory = useCallback(async (sid: string) => {
    const summaries = await apiRequest<MessageSummary[]>(
      config.api.endpoints.sessionMessages(sid)
    );
    const loaded: Message[] = summaries.map(s => ({
      id: s.id,
      role: s.role.toLowerCase() as 'user' | 'assistant',
      content: s.content,
  ...
  return { messages, streaming, pendingQuestion, sendMessage, submitAnswer, abort, setPendingQuestion, loadHistory };
}
