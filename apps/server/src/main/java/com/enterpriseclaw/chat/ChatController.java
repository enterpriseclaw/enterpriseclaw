...
public class ChatController {
    ...
    public ResponseEntity<List<MessageSummary>> getSessionMessages(
            @PathVariable String id,
            @RequestParam(defaultValue = "50") int limit,
            @RequestParam(defaultValue = "0") int offset) {
        return ResponseEntity.ok(chatService.getSessionMessages(id, limit, offset));
    }

    @DeleteMapping("/sessions/{id}/messages")
    public ResponseEntity<Void> clearSessionMessages(@PathVariable String id) {
        chatService.clearSessionMessages(id);
        return ResponseEntity.noContent().build();
    }
